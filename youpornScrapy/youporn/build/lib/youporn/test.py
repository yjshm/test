...
class YoupornPipeline(object):
    ...
    def close_spider(self, spider):
        ...
        # 保存类型RSS
        self.items.sort(key=itemgetter('class1','class2','class3'))  # 需要先排序，然后才能groupby。lst排序后自身被改变
        #items_group = groupby(self.items, itemgetter('class1'))
        # self.save_class_as_xml(items_group,'./rss/__youporn_categorie_rss.xml')

        struct_data = self.convert_struct(self.items)
        self.save_class_as_xml(struct_data, './rss/__youporn_categorie_rss.xml')
    ...
    '''
    def find_categores(self,catg):
                #一级类型，xpath不好分析，直接手查询
        PORN_CATEGORIES ={'Action': ['ACTION' ,'FINISH'],
                          'Story': ['STANDARD' ,'FAKE' ,'FANTASY' ,'TABOO'],
                          'Stars': ['ETHNICITY' ,'AGE' ,'BODY DETAILS' ,'HAIR','EXTRAS'],
                          'Details': ['ORIENTATION' ,'NUMBER' ,'PRODUCTION' ,'DETAILS']
                          }
        for key,items in PORN_CATEGORIES.items():
            if catg.upper() in items :
                print('{} in {}'.format(catg,key))
                return key
    '''
    ...
    def save_class_as_xml(self, rss, file_name):
        ...
        for class1_name,class1_item in rss.items() :
            ...
            #for class2_item in class1_item['data']:
            for class2_name, class2_item in class1_item.items():
                ...
                # item_data =class2_item['data']
                for class3_name, class3_item in class2_item.items():
                #for item_item in class2_item['data']:
...
with open('classout.json','r')as fp:
    strJson = fp.read()
    jsonData = json.loads(strJson)
    print(jsonData)
    youPornSave = YoupornPipeline()
    youPornSave.items = jsonData
    youPornSave.close_spider(None)